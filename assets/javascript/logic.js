...
$(document).ready(function () {
    ...
    var characters = {
        "Luke Skywalker": {
            ...
            health: 112,
            attack: 8,
            imageUrl: "assets/images/heroLS.jpg",
            enemyAttackBack: 15
        },
        "Darth Vader": {
            name: "Darth Vader",
            health: 100,
            attack: 14,
            imageUrl: "assets/images/heroDV.jpg",
            enemyAttackBack: 5
        },
        "Darth Raven": {
            name: "Darth Raven",
            health: 150,
            attack: 8,
            imageUrl: "assets/images/heroDR.jpg",
            enemyAttackBack: 20
        },
        "Darth Maul": {
            name: "Darth Maul",
            health: 180,
            attack: 7,
            imageUrl: "assets/images/heroDM.jpg",
            enemyAttackBack: 25
        }
    };
    ...
    var renderCharacter = function (character, renderArea) {
        ...
        var charCard = $("<div class='character card col center-align' data-name='" + character.name + "'>");
        var imgBox = $("<div class='card-image'>");
        var charImage = $("<img alt='image' class='character-image'>").attr("src", character.imageUrl);
        var charName = $("<div class='card-title flow-text'>").text(character.name);
        var charStats = $("<div class='card-action'>");
        var charHnA = $("<p class='flow-text'>").text(character.health + ' | ' + character.attack);
        charCard.append(imgBox).append(charImage).append(charName).append(charStats).append(charHnA);
        $(renderArea).append(charCard);
    };
    ...
    var restartGame = function (resultMessage) {
        ...
        var restart = $("<button class='waves-light btn red'>Restart</button>").click(function () {
            location.reload();
        });
        ...
        $("#fight-section").append(gameState);
        $("#fight-section").append(restart);
    };
        ...
        if ($("#defender").children().length !== 0) {
            ...
            if (defender.health > 0) {
                ...
                if (killCount >= combatants.length) {
                    clearMessage();
                    $("#attack-button").off("click");
                    restartGame("You truly have the FORCE! You Won!");
                }
            }
